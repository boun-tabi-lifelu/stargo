...
def propagate_go_preds(Y_hat, goterms, go_graph):
...
class Method(object):
    def __init__(self, method_name, pckl_fn, ontology, go_graph_path):
        annot = pickle.load(open(pckl_fn, 'rb'))
        self.Y_true = annot['Y_true']
        self.go_graph = obonet.read_obo(open(go_graph_path, 'r'))
    ...
    def _propagate_preds(self):
        self.Y_pred = propagate_go_preds(self.Y_pred, self.goterms, self.go_graph)
    ...
    def _cafa_go_aupr(self, labels, preds):
        ...
        for i in trange(0, n):
            ...
            for goterm in goterms[np.where(labels[i] == 1)[0]]:
                if goterm in self.go_graph:
                    all_gos = all_gos.union(nx.descendants(self.go_graph, goterm))
                all_gos.add(goterm)
        ...
        for t in trange(1, 100):
            ...
            for i in trange(0, n):
                ...
                for goterm in goterms[np.where(predictions[i] == 1)[0]]:
                    if goterm in self.go_graph:
                        pred_gos = pred_gos.union(nx.descendants(self.go_graph,
                                                             goterm))
                    pred_gos.add(goterm)
